...
World.prototype.setBlock = function(x, y, z, type) {
  ...
  y = Math.floor(y);
  z = Math.floor(z);
  console.log("Set block " + type + " at " + x + ", " + y + ", " + z);
...
World.prototype.isBlockOutOfBounds = function(x, y, z) {
  if (
    x < 0 ||
    y < 0 ||
    z < 0 ||
...
World.prototype.setFromString = function(string) {
  try {
...
if (typeof exports != "undefined") {
  ...
  World.prototype.loadFromFile = function(filename) {
    ...
    try {
      ...
      return this.setFromString(data);
    } catch (e) {
      return false;
    }
  };
  ...
  World.prototype.saveToFile = function(filename) {
    require("fs").writeFileSync(filename, this.getAsString());
  };
